...
class Zend_View_Helper_Navigation_Menu extends Zend_View_Helper_Navigation_HelperAbstract {
    ...
    protected function _renderDeepestMenu(Zend_Navigation_Container $container, $ulClass, $indent, $minDepth, $maxDepth) {
        ...
        $html    = $indent . '<ul' . $ulClass . ' 2>' . self::EOL;
    ...
    protected function _renderMenuLeft(Zend_Navigation_Container $container, $ulClass, $indent, $minDepth, $maxDepth, $onlyActive) {
        $html = '';
    ...
    protected function _renderMenu(Zend_Navigation_Container $container, $ulClass, $indent, $minDepth, $maxDepth, $onlyActive) {
        ...
        foreach ($iterator as $page) {
            ...
            if ($depth > $prevDepth) {
                ...
                if ((strpos($page->getHref(), 'filial') && (trim($ulClass) != 'class="menu-left"'))) {
                    $html .= $myIndent . '<ul class="two-col">' . self::EOL;
                } else {
                    $html .= $myIndent . '<ul' . $ulClass . ' >' . self::EOL;
                }
            } else if ($prevDepth > $depth) {
    ...
    public function renderMenu(Zend_Navigation_Container $container = null, array $options = array()) {
        ...
        public function renderMenuLeft(Zend_Navigation_Container $container = null, array $options = array()) {
        if (null === $container) {
            $container = $this->getContainer();
        }
        ...
        if ($options['onlyActiveBranch'] && !$options['renderParents']) {
            ...
            $html = $this->_renderMenuLeft($container, $options['ulClass'], $options['indent'], $options['minDepth'], $options['maxDepth'], $options['onlyActiveBranch']);
        }
